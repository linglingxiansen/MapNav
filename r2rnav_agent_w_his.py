...
class HM3D_Objnav_Agent:
    ...
    def step(self,planner_inputs):
        ...
        if not self.env.episode_over:
            ...
            cv2.imwrite(f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep}.png',self.obs['rgb'][:,:,::-1])
            ...
            images= [f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep}.png',f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/sem_labels{self.rank}-{args.exp_name}.png']
            ...
            if special_token :
            ...
            elif his_rgb and self.timestep>=2:
                
                images= [
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-8}.png',
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-7}.png',
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-6}.png',
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-5}.png',
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-4}.png',
                        #  f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-3}.png',
                         f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-2}.png',
                         f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep-1}.png',
                         f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep}.png',
                         f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/sem_labels{self.rank}-{args.exp_name}.png']
                ...
                prompt = (
                    ...
                    f"Your task is to follow this instruction:{self.obs['instruction']['text']}.\n"
                    f"Available observations:\n"
                    f"1. 2 historical RGB frames from previous timesteps\n2. Current RGB image showing the current egocentric view\n"
                    # f"1. RGB image showing the current egocentric view.\n"
                    f"3. Current Top-down semantic map, this map has been gradually built from all observations collected during your navigation process since the beginning.\n"
                    # f"   - As shown, this semantic map including objects such as {object_str}. \n"
                    f"   - Dark gray areas represent obstacles, light gray areas indicate traversable spaces, and white areas show unexplored regions. \n"
                    f"   - The red arrow shows your current position, orientation and the red line represents your past trajectory.\n"
                    # f"{objects_str}.\n"
                ...
                if objects:
                    objects_str = "   - As shown, this semantic map includes objects such as: " + ", ".join(objects)
                images= [f'/mnt/hpfs/baaiei/habitat/InstructNav_r2r_lf/inference/rgb{self.rank}-{args.exp_name}-{self.timestep}.png']
                prompt = (
                    f"<image>\n"
                    f"You are an intelligent agent in an indoor navigation task. "
                    f"Your task is to follow this instruction:{self.obs['instruction']['text']}.\n"
            ...
            if self.env.episode_over and args.collect == 1:
                ...
                if self.metrics['success']==1 or self.metrics['success']==0 :
